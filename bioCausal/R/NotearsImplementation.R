...
#' @param no_edge a 0,1 matrix where 1 specifies no_edge.
...
#'
#' @export
notearsInterceptMultiLoss = function(X, lambda1=0.1, losses=NULL, no_parents=NULL, no_edge=NULL,
...
notearsInterceptMultiLossNAs = function(X, lambda1=0.1, losses=NULL, no_parents=NULL,
										max.iter=100, h.tol=1e-6, rho.max=1e+6, w.threshold=0.01,
										seed=1) {


	#variables are in columns of X
	Weights=matrix(nrow=nrow(X), ncol=ncol(X))
	Weights[]=1
	Weights[is.na(X)]=0
	ss=colSums(Weights)

	#now ok to zero out X so inner products work
	X[is.na(X)]=0

	#normalize the weights per feature
	#this may or may not make sense
	#probably should be an option
	Weights=sweep(Weights, 2, nrow(Weights)/ss)
	...
	loss.func = function(W) { # ?
		...
		for(i in 1:ncol(X)){
			...
			if (loss.type=='l2') {
				...
				loss =loss+ (0.5 / n * sum(R[,i]*Weights[,i] ** 2))
			} else if (loss.type=='logistic') {
				loss =loss+ (1.0 / n * sum(log(sum(exp(M[,i]*Weights[,i])+1)) - X[,i] * M[,i]*Weights[,i]))
			} else if (loss.type=='poisson') {
				Si=exp(M[,i])
				loss=loss+ (1.0 / n * sum(Si - X[,i] * M[,i]*Weights[,i]))
			}
		}
		return(loss)
	}
	...
	G.loss.func = function(W) { # ?
		...
		for(i in 1:ncol(X)){
			...
			if (loss.type=='l2') {
				...
				G.loss[,i] = -1.0 / n * t(X) %*% (R[,i]*Weights[,i])
			} else if (loss.type=='logistic') {
				G.loss[,i] = 1.0 / n * t(X) %*% ((1.0 / (1 + exp(-1 * M[,i]*Weights[,i]))) - X[,i]*Weights[,i])
			} else if (loss.type=='poisson') {
				Si = exp(M[,i])
				G.loss[,i] = 1.0 / n * t(X) %*% ((Si - X[,i])*Weights[,i])
			}
		}
		return(G.loss)
	}
