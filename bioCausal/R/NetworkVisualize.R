...
#'
#' @export
network_visualize <- function(graph, gvarType, gvarShape, truth = NULL, mcoord = NULL, topE=50, layout.name = "fruchtermanreingold", alpha.range = c(0.05,1), edge.weight = T, use.alpha=F, sym = F, label.color="white", seed=42){
	set.seed(seed)
	if (sym) {
				...
				if (!sym) {
                    if (!is.null(edgeLabel)) {
                        if (net.dat[i,"edgeOpt"] == "falsePositive") {
                            ...
                            p <- p+geom_edges(data = net.dat[i,,drop = F], aes(x=x, y = y, xend = xend, yend = yend), size = net.dat[i,"weights.inuse"]+0.5, curvature = 0.3, color="#009300",arrow = arrow(length = unit(9, "pt"), type = "closed"))
                        } else if (net.dat[i,"edgeOpt"] == "wrongDirection") {
                            p <- p+geom_edges(data = net.dat[i,,drop = F], aes(x=x, y = y, xend = xend, yend = yend), size = net.dat[i,"weights.inuse"]+0.5, curvature = 0.3, color="#e10000",arrow = arrow(length = unit(9, "pt"), type = "closed"))
                        }
                    } else {
                        if (net.dat[i,"weights"] > 0){
					...
					if (!is.null(edgeLabel)) {
                        if (net.dat[i,"edgeOpt"] == "falsePositive") {
                            ...
                            p <- p+geom_edges(data = net.dat[i,,drop = F], aes(x=x, y = y, xend = xend, yend = yend), size = net.dat[i,"weights.inuse"]+0.5, curvature = 0.3, color="#009300")
                        } else if (net.dat[i,"edgeOpt"] == "wrongDirection") {
                            p <- p+geom_edges(data = net.dat[i,,drop = F], aes(x=x, y = y, xend = xend, yend = yend), size = net.dat[i,"weights.inuse"]+0.5, curvature = 0.3, color="#009300")
                        }
                    }
